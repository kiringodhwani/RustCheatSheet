...
- <ins>Example where Associated Types are Best:</ins> **In the case of the `Iterator` trait, it makes sense to use an associated type** because for any given implementation the `Iterator` trait, we want the `next()` method to always return the same concrete type. **<ins>Consider the type `Vec<i32>`. When we implement `Iterator` for this type, its `next()` method should always yield an `i32`<ins>**. It **<ins>doesn't make sense for `Vec<i32>` to have multiple, distinct `Iterator` implementations where one yields i32s and another yields Strings</ins>**. Therefore, the `Item` type returned by `next()` is fixed for that particular iterator.
